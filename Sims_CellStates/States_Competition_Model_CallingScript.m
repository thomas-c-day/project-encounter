...
Simple_Random_Switch = 2;               % switch that choose whether to do a simulation that has no randomness (1), or has randomness/fluctuations (2 or any other number)
...
Mu      = [1; 1];                       % maximum growth rate of the two strategies [# divisions/doubling time] (left S1 singled celled, right S2 or multi-celled)
...
for mm = 1:Msims
    ...
    [n_frac, b_frac, ~, ~, NumInd] = RUN_SINGLE_SIMULATION_STATES(Nrounds, G_0, alpha, phi, 0, Mu, K_m, Delta, Kappa, DivideThresh, DeathThresh, FigViz, Simple_Random_Switch);
