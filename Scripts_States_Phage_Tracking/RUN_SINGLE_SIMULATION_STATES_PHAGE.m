function [n_frac, b_frac, n_track, b_track, NumInd, c_track, InfectionArray] = RUN_SINGLE_SIMULATION_STATES_PHAGE(Nrounds, G0_F, G0_P, R0_F, R0_P, alpha, phi, Dct, Mu, K_m, Delta, Kappa, CellDivisionThreshold, CellDeathThreshold, First_Infection_TrackYN, InfectProb, PhageDeathProb, PhageBurstSize, FigViz)
    ...
    % Initial conditions:
    n0 = [2*ones(1e2,1); ones(1e2,1)];
        % State 1 is the single-celled strategy, State 2 is the
        % multi-celled strategy. Half the individuals are of State 2 at the
        % start of teh competition.
    ...
    tt = 0;
    N_FRAC_CHECK = n_frac(1);
    while N_FRAC_CHECK > 0
        tt = tt + 1; 

        % Check to stop the simulation if over 72 hrs has passed
        if tt > 218
            break
        elseif b_frac(tt) == 1
            break;  % also break the sim if there is fixation
        end
        ...
        infection_TF = rand(size(q)) < q; % infection occurs stochastically with some probability
        ...
        % If the carrying capacity has been reached, randomly sample the
        % population without weights:
        if length(n_track{tt+1}) > Kappa
            ix = randsample(length(n_track{tt+1}), Kappa, false);
            n_track{tt+1} = n_track{tt+1}(ix);
            b_track{tt+1} = b_track{tt+1}(ix);
        end
